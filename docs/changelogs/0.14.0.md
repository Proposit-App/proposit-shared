...
`v0.14.0` lands the `@proposit/shared` half of the pipeline-status
UI initiative (Phase 2 slice 2F.A). One implementation commit on
the `pipeline-status/2F.A` branch:

- Slice 2F.A (`362bc1c`): pipeline-status schemas + api-client
  method pair + 51 tests + `./schemas/api/pipeline-status`
  package.json sub-entry export.

Plus the docs/release-prep commit landing these notes and the
`pnpm version` commit landing the version bump.

Minor release — additive only, no breaking changes. Lands the
wire-contract surface that `proposit-server` slice 2F's
pipeline-status UI consumes.
...
- **Add:** `docs/release-notes/0.14.0.md` — user-facing notes.
- **Add:** `docs/changelogs/0.14.0.md` — this file.
