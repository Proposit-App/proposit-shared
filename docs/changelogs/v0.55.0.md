...
<changes starting-hash="c3d9baf" ending-hash="ba2ccb3">
...
- **`@proposit/proposit-core` moved to `^3.4.0`** in both `peerDependencies` and
  `devDependencies`. Core 3.4.0 adds the Presentable rule code `P-6`, widening
  its `TGrammarRuleCode` union.
  `dist/schemas/api/grammar-violations.d.ts` bakes that union literally from
  whatever core it was built against, so a consumer typechecking against core
  3.4.0 could not assign a 3.4.0 `TViolation[]` into shared's narrower
  `P-1`..`P-5` union. Repinning and rebuilding re-widens the emitted union; no
