...
<changes starting-hash="caace51" ending-hash="aaeb008">
...
**Breaking, and the reason this is a minor rather than a patch.** Widening the
two overload return unions is a type-level change with no opt-out: a consumer
that hands `result.error` to anything typed as its own error union stops
type-checking on upgrade, until it adds an arm for the new envelope. Both
consumers hit exactly this. Per the pre-1.0 policy in `AGENTS.md`, a caret pin
does not cross a minor, so no existing consumer is upgraded into it by
surprise — but a deliberate repin must budget for the classifier arm in the same
commit as the version bump, or it lands on a red tree.
