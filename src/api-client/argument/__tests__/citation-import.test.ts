...
function clientWith(status: number, body: unknown) {
    ...
    return createApiClient({ baseUrl: "http://x", fetchImpl: fetchImpl as never })
}

describe("citationImport", () => {
    it("parses a full analysis", async () => {
        const c = clientWith(200, { websiteTitle: "N", pageTitle: "P", byline: "Jane Doe" })
        const r = await c.citationImport("https://x/a", "Website")
        expect(r.ok).toBe(true)
        if (r.ok) expect(r.value).toEqual({ websiteTitle: "N", pageTitle: "P", byline: "Jane Doe" })
    })
