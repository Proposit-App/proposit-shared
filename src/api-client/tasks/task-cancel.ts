// api-client method for the cancel endpoint on `proposit-server`.
// Cancel is a no-body DELETE with a single path param (the task id) — mirrors
// `deleteReactionImpl`. The route returns 200 + the UPDATED `argument_create`
// task (status `TaskStatus.CANCELLED`); wire shape comes from
// `@proposit/shared/schemas/api/task-cancel` (`CancelTaskResponse`).
//
...
// cancel DELETE — harmless here, and the consumer writes no
