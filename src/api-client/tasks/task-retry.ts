// api-client methods for the two retry endpoints on `proposit-server`.
// Retry is a whole-task re-run (core has no single-stage resume), so both
// routes are POSTs with path params only — no request body. Wire shape comes from
...
// member that cannot occur on a retry POST — harmless here, and the consumer
